...
public class Record_history extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        recyclerView = findViewById(R.id.rv_record_history);
        recyclerView.setHasFixedSize(true);

        timerRecordItems.clear();
        mDBHelper = new DBHelper(Record_history.this, "TABLE_SUBJECT_HISTORY");
        db = mDBHelper.getReadableDatabase();

        Cursor cursor = mDBHelper.LoadSQLiteDBCursor();
        try {
            cursor.moveToFirst();
            Log.d("TAG", "CHECK12: cursor의 수:"+cursor.getCount());
            while (!cursor.isAfterLast()) {
                addItem(cursor.getLong(0), cursor.getString(1), cursor.getString(2), cursor.getString(3));
                Log.d("TAG", "CHECK123"+cursor.getLong(0)+", "+cursor.getString(1)+", "+cursor.getString(2)+cursor.getString(3));
                cursor.moveToNext();
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        recyclerviewAdater = new MyAdapter(timerRecordItems, Record_history.this, "TABLE_SUBJECT_HISTORY");
